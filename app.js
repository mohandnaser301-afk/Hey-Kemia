// --- محرك منصة هي كيميا ! المتكامل المباشر ---

function initDatabase() {
  // 1. حساب المشرف العام وحسابات النظام
  ...
  // 2. الكورسات الافتراضية
  if (!localStorage.getItem("edu_courses")) {
    const defaultCourses = [
      {
        id: "c1",
        title: "كورس الباب الأول: العناصر الانتقالية والتأسيس",
        desc: "توزيع الإلكترونات، حالات التأكسد، والألوان وخواص الحديد ومركباته بالتفصيل مع التدريبات.",
        price: 220,
        maxViews: 5,
  ...
  if (!localStorage.getItem("edu_activity_logs")) {
    localStorage.setItem("edu_activity_logs", JSON.stringify([
      { id: Date.now(), actorName: "المشرف العام", actorRole: "SUPER_ADMIN", actionType: "تهيئة النظام", details: "تم تشغيل وتفعيل منصة هي كيميا ! بنجاح.", timestamp: new Date().toLocaleString('ar-EG') }
    ]));
  }
}
initDatabase();

// تسجيل العمليات باللوجات
...
function handleRegisterSubmit(event) {
  ...
  const eduTypeChecked = document.querySelector('input[name="educationType"]:checked');
  ...
  const newUser = {
    ...
    educationType: eduTypeChecked ? eduTypeChecked.value : "GENERAL",
    role: "STUDENT",
    enrolledCourses: [],
    courseAccessCount: {}
  };
  ...
  users.push(newUser);
  localStorage.setItem("edu_users", JSON.stringify(users));
  alert("🎉 تم إنشاء حسابك بنجاح في منصة هي كيميا ! يمكنك الآن تسجيل الدخول.");
  window.location.href = "login.html";
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (lForm) lForm.addEventListener("submit", handleLoginSubmit);
  const rForm = document.getElementById("registerForm");
  if (rForm) rForm.addEventListener("submit", handleRegisterSubmit);
});